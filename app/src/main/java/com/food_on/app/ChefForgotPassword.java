                ...
                FAuth.sendPasswordResetEmail(forgetpassword.getEditText().getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            mDialog.dismiss();
                            ReusableCodeForAll.ShowAlert(ChefForgotPassword.this, "", "Ο κωδικός στάλθηκε με Email");
                        } else {
                            mDialog.dismiss();
                            ReusableCodeForAll.ShowAlert(ChefForgotPassword.this, "Λάθος", task.getException().getMessage());
                        }
                    }
                });
            }
