...
public class CustomerPassword extends AppCompatActivity {
                                        ...
                                        user.updatePassword(ne).addOnCompleteListener(new OnCompleteListener<Void>() {
                                            ...
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()) {
                                                    ...
                                                    Toast.makeText(CustomerPassword.this, "Ο κωδικός άλλαξε", Toast.LENGTH_SHORT).show();
                                                } else {
    ...
    public boolean isvalid() {
        ...
        if (TextUtils.isEmpty(ne)) {
            ...
            neww.setError("Απαιτείται ");

        } else {
            if (ne.length() < 6) {
                neww.setErrorEnabled(true);
                neww.setError("Αδύναμος κωδικός");
        ...
        if (TextUtils.isEmpty(conf)) {
            ...
            confirm.setError("Απαιτείται επιβεβαίωση ");


        } else {
            if (!ne.equals(conf)) {
                neww.setErrorEnabled(true);
                neww.setError("Ο κωδικός δεν ταιριάζει");
                confirm.setError("Ο κωδικός δεν ταιριάζει");
            } else {
