...
public class DeliveryShipOrderFragmentAdapter extends RecyclerView.Adapter<DeliveryShipOrderFragmentAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.grandtotalprice.setText("Σύνολο: € " + deliveryShipFinalOrders1.getGrandTotalPrice());
        holder.mobilenumber.setText("+30" + deliveryShipFinalOrders1.getMobileNumber());
        ...
        holder.ShipOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                FirebaseDatabase.getInstance().getReference("CustomerFinalOrders").child(userid).child(random).child("OtherInformation").child("Status").setValue("Η παραγγελία σου είναι στον δρόμο...").addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        FirebaseDatabase.getInstance().getReference().child("Tokens").child(userid).child("token").addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                ...
                                sendNotifications(usertoken, "Eκτιμώμενη ώρα άφιξης", "Η παραγγελία σου παραλαμβάνετε απο το Ντελίβερι, είναι στον δρόμο", "DeliverOrder");
                            }
            ...
            public void onResponse(Call<MyResponse> call, Response<MyResponse> response) {
                if (response.code() == 200) {
                    if (response.body().success != 1) {
                        Toast.makeText(context, "Αποτυχία", Toast.LENGTH_SHORT).show();
                    }
                }
            }
