                ...
                FAuth.sendPasswordResetEmail(forgetpassword.getEditText().getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            mDialog.dismiss();
                            ReusableCodeForAll.ShowAlert(Delivery_ForgotPassword.this, "", "Ο κωδικός έχει σταλέι στο Email");
                        } else {
                            mDialog.dismiss();
                            ReusableCodeForAll.ShowAlert(Delivery_ForgotPassword.this, "Λάθος", task.getException().getMessage());
                        }
                    }
                });
            }
