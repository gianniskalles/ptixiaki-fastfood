                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    if (FAuth.getCurrentUser().isEmailVerified()) {
                                        Toast.makeText(Login.this, "Είσαι συνδεδεμένος", Toast.LENGTH_SHORT).show();
                                        Intent z = new Intent(Login.this, CustomerFoodPanel_BottomNavigation.class);
                                        startActivity(z);
                                        finish();
                                    } else {
                                        ReusableCodeForAll.ShowAlert(Login.this,"","Επιβεβαίωσε το Email σου");
                                    }

                                } else {

                                    mDialog.dismiss();
                                    ReusableCodeForAll.ShowAlert(Login.this,"Λάθος",task.getException().getMessage());
                                }
                            }
                        });
    ...
    public boolean isValid() {
        ...
        if (TextUtils.isEmpty(em))
        {
            ...
            if (em.matches(emailpattern))
            {
                ...
                email.setErrorEnabled(true);
                email.setError("Βάλε ένα σωστό Email");
            }
